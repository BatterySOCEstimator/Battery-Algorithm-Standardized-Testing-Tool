...
def obtain_output_data(setups, data, inputs):
    """
    ...
    sizes = [len(data[setup].cycle) for setup in setups]
    max_size = max(sizes)
    max_idx = sizes.index(max_size)
    
    # Find size for array without charge and other cycles
    cycle_names = [str(name) for name in data[setups[max_idx]].cycle.iloc[:, 0]]
    ...
    for t, setup_name in enumerate(setups): # Repeat for all vehicle weight and settings
        ...
        file_data = data[setup_name].cycle
        
        # Get cycle information
        cycle_names_list = [str(name) for name in file_data.iloc[:, 0]]
        temps_list = [float(temp) for temp in file_data.iloc[:, 1]]
        ...
        for i in range(len(cycle_names_list)):
            ...
            # Get actual SOC
            cycle_data = file_data.Data[i]
            soc_act = np.array(cycle_data['SOC'])
            
            # Predict SOC
            soc_pred, time_elapsed, n_samp = predict_soc(inputs, file_data, i)
    ...
    file_data_dict = {setup: data[setup].cycle for setup in setups}
