...
def create_figures(user_model, output_data, inputs, data, file_data, processing_folder, rmse, temperatures):
    """
    ...
    create_charging_figures(output_data, indices, inputs, file_data, user_model)
    ...
    # Robustness tests
    rmse_soc_offset = create_initial_soc_figures(output_data, indices, inputs, data, file_data, user_model)
    rmse_sens_offset = create_sensor_offset_figures(output_data, indices, inputs, file_data, user_model)
...
def find_file_index(file_data, setup, cycle_substr, temp):
    ...
    for i in range(len(df)):
        name = str(df[i, 0]) # Cycle name
        cycle_temp = float(df[i, 1]) # Temperature
...
def create_charging_figures(output_data, indices, inputs, file_data, user_model):
    ...
    cycle_data = file_data['m80'][charge_idx][3]
    ...
    for j, input_name in enumerate(inputs):
        X1[:, j] = np.array(getattr(cycle_data, input_name))
...
def create_initial_soc_figures(output_data, indices, inputs, data, file_data, user_model):
    ...
    for col, (idx_key, setup, title) in enumerate(test_cases):
        ...
        for isoc in isoc_values:
            ...
            soc_pred, _, _ = predict_soc(inputs, file_data[setup], file_idx, user_model, 0, isoc_idx)
            
            # Align lengths
            min_len = min(len(soc_act[isoc_idx:]), len(soc_pred))
            soc_pred = soc_pred[:min_len]
            soc_act_trimmed = soc_act[isoc_idx:][:min_len]
            
            # Calculate RMSE from offset point
            rmse_val = 100 * np.sqrt(np.mean((soc_act_trimmed - soc_pred)**2))
...
def create_sensor_offset_figures(output_data, indices, inputs, file_data, user_model):
    ...
    for col, (idx_key, setup, title) in enumerate(test_cases):
        ...
        for offset in offsets:
            file_idx = find_file_index(file_data, setup, idx_key.split('_')[2], 
                                      output_data[setup].iloc[idx, 0])
            soc_pred, _, _ = predict_soc(inputs, file_data[setup], file_idx, user_model, offset)
