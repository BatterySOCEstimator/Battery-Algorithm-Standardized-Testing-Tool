 ...
 *
 * Files are saved to `UPLOAD_DIR/{userId}/{storageId}/`, where `storageId` is
 * generated by {@link uploadMiddleware} and attached to `req` before Multer
 * runs. The directory is created if it does not already exist.
 *
...
const storage = multer.diskStorage({
  destination: (req, _file, cb) => {
    ...
    // Save files to uploads/{userId}/{storageId} — must match the path the
    // controller stores in the DB (models.filePath), or evaluation later
    // can't find the files it just uploaded.
    const dir = path.join(process.env.UPLOAD_DIR ?? './uploads', userId, storageId);
